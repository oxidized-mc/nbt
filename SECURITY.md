...
- Parsing vulnerabilities (malformed NBT data causing panics or crashes)
- Decompression bombs (small gzip/zlib payload expanding to excessive memory)
- Memory safety issues (should be none — the crate enforces `#![deny(unsafe_code)]`)
- Denial of service via pathological input (deeply nested tags, unbounded allocations)
