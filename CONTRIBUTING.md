...
```bash
# 1. Fork and clone
git clone https://github.com/oxidized-mc/nbt.git
cd nbt
...
cargo install cargo-fuzz    # fuzz testing (requires nightly)
```
...
`oxidized-nbt` is a library crate in the
...
Use `nbt` as the scope for all changes to this crate.
